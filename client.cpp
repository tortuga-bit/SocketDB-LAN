...
int main() {
  ...
  { 
    ...
    memset(&serv_addr, 0, sizeof(serv_addr));// Limpia la estructura de dirección del servidor para evitar datos residuales

    serv_addr.sin6_family = AF_INET6;
    serv_addr.sin6_port = htons(PORT);

    // Convert IPv6 address from text to binary form
    if (inet_pton(AF_INET6, "2806:106e:1a:ffe5:f932:4933:a2b2:6749", &serv_addr.sin6_addr) <= 0) {
    ...
    if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
      ...
      std::cout << "Echo mensaje" << std::endl;
